...
model = coatnet.CoAtNet((64, 64), 1, num_blocks, channels, num_classes=26)

optimiser = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)
criterion = torch.nn.CrossEntropyLoss()
scheduler = torch.optim.lr_scheduler.LinearLR(optimiser, total_iters=10)
...
def train(model, device, train_loader, optimiser, criterion, scheduler, epoch):
    model.train()
    running_loss = 0.0
...
def run():
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        train_loss = train(model, device, train_loader, optimiser, criterion, scheduler, epoch)
